"""Tests for bootstrap aggregating in BaggingImputer."""
...
def test_each_run_gets_a_sorted_bootstrap_sample_and_its_own_seed(
    ...
    BaggingImputer(
        base_imputer=RecordingImputer(), n_estimators=4, max_samples=0.8, random_state=0
    ).impute(frame)
...
def test_is_reproducible_with_random_state(frame: pd.DataFrame) -> None:
    base = PMMImputer(k=3, random_state=0)
    first = BaggingImputer(base_imputer=base, n_estimators=5, random_state=1).impute(
        frame
    )
    second = bagging_impute(frame, base_imputer=base, n_estimators=5, random_state=1)
    pd.testing.assert_frame_equal(first, second)


def test_averages_differ_from_single_runs() -> None:
...
def test_reduces_error_of_a_high_variance_imputer() -> None:
...
def test_empty_column_stays_missing(frame: pd.DataFrame) -> None:
...
@pytest.mark.parametrize("cls", _seeded_imputer_classes(), ids=lambda cls: cls.__name__)
...
def test_with_random_state_copies_deterministic_imputers() -> None:
