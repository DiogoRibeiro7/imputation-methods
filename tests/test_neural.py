"""Tests for the neural-network imputers."""
...
class TestAutoencoderImputer:
    """Tests for ``AutoencoderImputer``."""

    def test_autoencoder_imputer(self) -> None:
    ...
    def test_autoencoder_different_architectures(self) -> None:
...
class TestGAINImputer:
    """Tests for ``GAINImputer``."""

    def test_beats_mean_imputation_on_correlated_data(self) -> None:
...
class TestMLP:
    """Tests for the NumPy network that ``GAINImputer`` trains."""

    def test_mlp_gradients_match_finite_differences(self) -> None:
        ...
        def loss(inputs: np.ndarray) -> float:
            ...
            return float(
                -np.mean(target * np.log(prob) + (1 - target) * np.log(1 - prob))
            )
