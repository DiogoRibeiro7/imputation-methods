...
and saves them to the figures/ directory for use in documentation.
"""
...
plt.tight_layout()
plt.savefig('figures/01_missingness_pattern.png')
plt.close()
...
rmse_scores = {
    name: rmse(original, imp_df)
    for name, imp_df in imputed_data.items()
}
mae_scores = {
    name: mae(original, imp_df)
    for name, imp_df in imputed_data.items()
}
...
bars1 = ax1.bar(methods, rmse_vals, color=['#3498db', '#e74c3c', '#2ecc71',
                                            '#f39c12', '#9b59b6'])
ax1.set_ylabel('RMSE', fontsize=12)
ax1.set_title('Root Mean Squared Error by Method', fontsize=14,
              fontweight='bold')
...
bars2 = ax2.bar(methods, mae_vals, color=['#3498db', '#e74c3c', '#2ecc71',
                                           '#f39c12', '#9b59b6'])
...
plt.tight_layout()
plt.savefig('figures/02_error_comparison.png')
plt.close()
...
axes[0].hist(original[feature_name], bins=30, alpha=0.7,
...
for idx, (name, imp_df) in enumerate(imputed_data.items(), start=1):
    axes[idx].hist(imp_df[feature_name], bins=30, alpha=0.7,
                   edgecolor='black')
...
plt.suptitle(f'Distribution Comparison: {feature_name}', fontsize=16,
             fontweight='bold', y=1.00)
plt.tight_layout()
plt.savefig('figures/03_distribution_comparison.png')
plt.close()
...
for idx, (name, imp_df) in enumerate(imputed_data.items()):
    ...
    axes[idx].plot([original_vals.min(), original_vals.max()],
...
plt.suptitle('Imputed vs Original Values (Missing Data Only)', fontsize=16,
             fontweight='bold', y=1.00)
plt.tight_layout()
plt.savefig('figures/04_scatter_comparison.png')
plt.close()
...
plt.title('Imputation Methods Performance Summary', fontsize=14,
          fontweight='bold', pad=20)
plt.savefig('figures/05_summary_table.png')
plt.close()
...
print("="*60)
print(f"\nGenerated {len(os.listdir('figures')) - 1} visualization files")
