...
def generate_housing_data(n_samples=1000, missing_rate=0.20):
    ...
    bedrooms = np.clip(np.round(square_feet / 500 + np.random.normal(0, 0.5, n_samples)), 1, 6)
    ...
    feature_cols = ['square_feet', 'bedrooms', 'bathrooms', 'age_years', 'lot_size', 'garage_size']
...
def main():
    ...
    axes[0, 0].set_title('Test Set RMSE (Lower is Better)', fontsize=12, fontweight='bold')
    ...
    width = 0.35
    axes[1, 0].bar(x - width/2, results_df['train_rmse'], width, label='Train', alpha=0.8)
    axes[1, 0].bar(x + width/2, results_df['test_rmse'], width, label='Test', alpha=0.8)
    ...
    for idx, row in results_df.iterrows():
        axes[1, 1].annotate(row['name'], (row['train_r2'], row['test_r2']),
