...
def generate_test_data(n_rows: int, n_cols: int, missing_rate: float, seed: int = 42) -> tuple:
...
def run_benchmark_suite(
    ...
    for method_name, imputer in methods.items():
        ...
        if result['success']:
            print(f"✓ {result['time']:6.2f}s  RMSE: {result['rmse']:.4f}")
        else:
...
def main():
    ...
    methods_slow = {
        ...
        'Autoencoder': AutoencoderImputer(hidden_layer_sizes=(10,), max_iter=100, random_state=42),
...
def generate_visualizations(df_results: pd.DataFrame):
    ...
    ax1.barh(scenario_data_sorted['method'], scenario_data_sorted['time'], color='steelblue')
    ax1.set_xlabel('Time (seconds)', fontsize=11)
    ax1.set_title('Execution Time Comparison (Medium Dataset)', fontsize=12, fontweight='bold')
    ax1.set_xscale('log')
    ...
    for idx, row in df_success.iterrows():
        if row['time'] < 1 or row['rmse'] < 0.5:  # Label interesting points
            ax3.annotate(row['method'], (row['time'], row['rmse']),
    ...
    for method in df_success['method'].unique():
        ...
        if len(method_data) > 1:
            ax4.plot(method_data['n_rows'], method_data['time'],
    ...
    missing_rates = small_scenarios.groupby(['method', 'missing_rate'])['rmse'].mean().reset_index()
    for method in missing_rates['method'].unique():
        method_data = missing_rates[missing_rates['method'] == method]
        ax5.plot(method_data['missing_rate'], method_data['rmse'],
    ...
    ax6.set_title('Performance Heatmap (RMSE by Method and Scenario)',
...
def print_summary(df_results: pd.DataFrame):
    ...
    print(f"Successful: {len(df_success)} ({len(df_success)/len(df_results)*100:.1f}%)")
    print(f"Failed: {len(df_results) - len(df_success)}")

    # Top 5 fastest methods
    print("\n📊 TOP 5 FASTEST METHODS (Medium Dataset):")
    medium_scenario = df_success[df_success['scenario'] == 'Medium - Medium Missing']
    ...
    best_overall = medium_scenario.nsmallest(5, 'score')[['method', 'time', 'rmse', 'score']]
    for idx, row in best_overall.iterrows():
        print(f"  {row['method']:20s}: {row['time']:6.2f}s, RMSE {row['rmse']:.4f} (score: {row['score']:.3f})")
